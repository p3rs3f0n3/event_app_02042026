...
import { normalizeEventDto, normalizeEventDtoList } from '../utils/eventDto';
...
export const saveExpoPushToken = async ({ userId, expoPushToken }) => withApiErrorHandling(async () => (
  await getAPI().post('/notifications/token', { userId, expoPushToken })
).data);

export const sendExpoPushNotification = async (data) => withApiErrorHandling(async () => (
  await getAPI().post('/notifications/send', data)
).data);
