...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, SafeAreaView, ActivityIndicator, Modal, Alert, ScrollView, KeyboardAvoidingView, Platform } from 'react-native';
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          <View style={styles.header}>
            <Text style={[styles.title, { fontSize: metrics.titleSize }]}>{appConfig?.appName || 'EVENTAPP'}</Text>
          </View>

          <View style={[styles.card, { padding: metrics.cardPadding }]}>
            <Text style={[styles.subtitle, { fontSize: metrics.subtitleSize }]}>BIENVENIDO</Text>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 36, fontWeight: 'bold', color: '#FFF', letterSpacing: 4, textAlign: 'center' },
  card: { backgroundColor: 'rgba(255,255,255,0.1)', padding: 24, borderRadius: 25, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.3, shadowRadius: 10, elevation: 10 },
  subtitle: { fontSize: 24, fontWeight: 'bold', color: '#FFB300', marginBottom: 22, textAlign: 'center' },
